...
const INDUSTRIES = [
  ...
  "Energy & Utilities", "Manufacturing", "Government & Public Sector",
  "Consulting", "Media & Communications", "Education & Research",
  "Venture Capital & Private Equity", "Other",
];

function AccessGate({ onUnlock, dark }) {
  const [form, setForm] = useState({ firstName: "", lastName: "", email: "", industry: "", jobTitle: "" });
  const [errors, setErrors] = useState({});
  const [submitting, setSubmitting] = useState(false);

  const accent = dark ? "#b99eff" : "#7a3fd1";
  const bg     = dark ? "#06020f" : "#f8f7fc";
  const card   = dark ? "#0d0820" : "#ffffff";
...
async function handleSubmit(e) {
  ...
  const field = (key, label, type = "text", opts = null) => (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
      <label style={{ fontSize: "0.72rem", fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", opacity: 0.5 }}>
        {label}
      </label>
      {opts ? (
        <select
          ...
          style={{
            padding: "0.65rem 0.85rem", borderRadius: "8px", fontSize: "0.88rem",
            background: dark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.03)",
            border: `1.5px solid ${errors[key] ? "#f57eb3" : border}`,
            color: form[key] ? text : (dark ? "rgba(255,255,255,0.3)" : "rgba(0,0,0,0.3)"),
        ...
        <input
          type={type} value={form[key]} placeholder={`Enter ${label.toLowerCase()}…`}
          onChange={e => { setForm(f => ({ ...f, [key]: e.target.value })); setErrors(er => ({ ...er, [key]: null })); }}
      ...
      {errors[key] && <span style={{ fontSize: "0.7rem", color: "#f57eb3" }}>{errors[key]}</span>}
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        backgroundImage: `linear-gradient(${dark?"rgba(185,158,255,0.6)":"rgba(122,63,209,0.6)"} 1px, transparent 1px), linear-gradient(90deg, ${dark?"rgba(185,158,255,0.6)":"rgba(122,63,209,0.6)"} 1px, transparent 1px)`,
      ...
      <motion.div
        ...
        style={{
          ...
          boxShadow: dark ? "0 24px 80px rgba(0,0,0,0.6)" : "0 16px 60px rgba(0,0,0,0.1)",
          padding: "2.2rem 2rem",
        }}
      >
        {/* Logo / header */}
        <div style={{ textAlign: "center", marginBottom: "1.8rem" }}>
          ...
          <h2 style={{
            fontFamily: "'Orbitron', sans-serif", fontSize: "1.25rem",
            fontWeight: 900, marginBottom: "0.4rem", color: text,
          }}>
            TTFC 2026 Agenda
          </h2>
          <p style={{ fontSize: "0.85rem", opacity: 0.5, lineHeight: 1.5 }}>
            Please identify yourself to access the full agenda.
          </p>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          {field("industry", "Industry", "text", INDUSTRIES)}
          {field("jobTitle", "Job Title")}

          <motion.button
            whileHover={{ y: -1, boxShadow: `0 10px 28px ${accent}44` }}
            whileTap={{ scale: 0.97 }}
            onClick={handleSubmit}
            disabled={submitting}
            style={{
              marginTop: "0.4rem", padding: "0.8rem",
              borderRadius: "10px", border: "none", cursor: submitting ? "wait" : "pointer",
              fontFamily: "'Orbitron', sans-serif", fontWeight: 700,
              fontSize: "0.82rem", letterSpacing: "0.06em",
              background: submitting ? (dark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.08)") : `linear-gradient(135deg,${accent},#f5a623)`,
              color: submitting ? (dark ? "rgba(255,255,255,0.4)" : "rgba(0,0,0,0.3)") : "#fff",
              transition: "background 0.2s",
            }}
          >
            {submitting ? "UNLOCKING…" : "ACCESS AGENDA →"}
          </motion.button>
        </div>

        <p style={{ textAlign: "center", fontSize: "0.68rem", opacity: 0.28, marginTop: "1.2rem", lineHeight: 1.5 }}>
          This agenda is confidential. By proceeding you agree not to share its contents.
        </p>
      </motion.div>
    </div>
  );
        ...
        <div aria-hidden style={{ position: "fixed", inset: 0, zIndex: 9999, pointerEvents: "none", overflow: "hidden" }}>
          {Array.from({ length: 10 }).map((_, i) => (
            <div key={i} style={{
              ...
              color: dark ? "rgba(185,158,255,0.05)" : "rgba(122,63,209,0.04)",
              userSelect: "none",
            }}>
              {visitor.firstName} {visitor.lastName} · {visitor.email} · TTFC 2026
            </div>
          ))}
        </div>
      )}
