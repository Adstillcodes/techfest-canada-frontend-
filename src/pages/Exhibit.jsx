...
function BoothGallery({ images, isDark, border, cardBg }) {
  ...
  return (
    <div style={{ borderRadius: 24, overflow: "hidden", aspectRatio: "4/3", border: "1px solid " + border, background: cardBg, position: "relative" }}>
      <AnimatePresence mode="sync">
        {images.map(function (src, i) {
          return i === active ? (
            <motion.img key={src} src={src} alt={"Booth view " + (i + 1)}
      ...
      <div style={{ position: "absolute", bottom: 14, left: 0, right: 0, zIndex: 5, display: "flex", justifyContent: "center", gap: 8 }}>
        {images.map(function (_, i) {
          return (
            <button key={i} onClick={function () { handleDot(i); }}
              style={{ width: i === active ? 22 : 8, height: 8, borderRadius: 999, border: "none", cursor: "pointer", background: i === active ? "#f5a623" : "rgba(255,255,255,0.45)", transition: "all 0.3s ease", padding: 0 }}
            />
          );
        })}
      </div>
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 60, zIndex: 3, background: "linear-gradient(to top, rgba(0,0,0,0.35), transparent)", pointerEvents: "none", borderRadius: "0 0 24px 24px" }} />
    </div>
  );
}
  ...
  return (
    <div style={{ background: bg, minHeight: "100vh", color: textMain, overflowX: "hidden" }}>
      <style>{`
        ...
        @keyframes aurora-flow { from { background-position: 50% 50%, 50% 50%; } to { background-position: 350% 50%, 350% 50%; } }
        .aurora-layer {
          position: absolute; inset: -10px; pointer-events: none;
          will-change: transform; background-size: 300%, 200%;
          background-position: 50% 50%; filter: blur(10px); opacity: 0.50;
      ...
      {/* HERO */}
      <section style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", overflow: "hidden", background: isDark ? "#06020f" : "#f4f0ff", paddingTop: "clamp(120px, 15vw, 160px)", paddingBottom: "20px" }}>
        <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
          <div className={isDark ? "aurora-layer aurora-layer--dark" : "aurora-layer aurora-layer--light"} />
        </div>
        <div style={{ position: "absolute", inset: 0, pointerEvents: "none", background: isDark ? "radial-gradient(ellipse 80% 60% at 50% 50%, transparent 20%, #06020f 100%)" : "radial-gradient(ellipse 80% 60% at 50% 50%, transparent 20%, #f4f0ff 100%)" }} />
        <motion.div style={{ position: "relative", zIndex: 10, textAlign: "center", padding: "0 5%", maxWidth: 900, margin: "0 auto" }}
          initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}
        >
          <h1 style={{ fontFamily: "'Orbitron', sans-serif", fontSize: "clamp(3rem, 7vw, 5rem)", fontWeight: 900, lineHeight: 1.1, marginBottom: 24, letterSpacing: "-0.5px", color: textMain }}>
            Exhibit{" "}
            <span style={{ background: "linear-gradient(135deg, #7a3fd1, #f5a623)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>With Us</span>
          </h1>
      ...
      {/* INTRO */}
      <motion.section initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.8 }}
        style={{ padding: "20px 5% 80px", maxWidth: 1000, margin: "0 auto", position: "relative", zIndex: 10 }}
      >
        <div style={{ display: "flex", flexDirection: "column", gap: 24, fontSize: "1.1rem", color: textMuted, lineHeight: 1.8, textAlign: "justify" }}>
          <p>Exhibiting here is not just about taking space on a show floor. It is about claiming visibility in front of a high value audience that matters. It is about creating direct conversations, building market credibility, launching solutions, generating qualified leads, and positioning your company as a serious player in the future of technology.</p>
        </div>
      </motion.section>
      ...
      <div style={{ paddingBottom: "80px" }}>
        {BOOTH_TIERS.map((tier, index) => (
          <BoothRow key={tier.id} tier={tier} isDark={isDark} textMain={textMain} textMuted={textMuted} border={border} cardBg={cardBg} index={index} onOpenModal={() => setSelectedBooth(tier)} />
        ))}
      </div>
      ...
      <section style={{ padding: "0 5% 120px", maxWidth: 1200, margin: "0 auto" }}>
        ...
        >
          ...
          <div style={{ padding: "clamp(40px, 6vw, 64px)", display: "flex", flexDirection: "column", justifyContent: "center" }}>
            ...
            <p style={{ color: textMuted, lineHeight: 1.7, fontSize: "1.1rem", marginBottom: 36, maxWidth: 480 }}>Join the ecosystem of innovators and leaders. Partner with us or secure your delegate pass today.</p>
            <div className="cta-row" style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              <motion.a href="/sponsor" className="btn-outline" whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
                style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "16px 36px", borderRadius: 14, fontFamily: "'Orbitron', sans-serif", fontWeight: 800, fontSize: "0.9rem", textDecoration: "none", letterSpacing: "0.5px", transition: "all 0.25s ease" }}
              >Partner With Us</motion.a>
      ...
      <AnimatePresence>
        {selectedBooth && (
          <EnquireModal booth={selectedBooth} onClose={() => setSelectedBooth(null)} isDark={isDark} textMain={textMain} border={border} cardBg={cardBg} />
        )}
      </AnimatePresence>
...
function BoothRow({ tier, isDark, textMain, textMuted, border, cardBg, index, onOpenModal }) {
  const hasBg = index % 2 === 0;
  return (
    <section style={{ padding: "clamp(60px, 8vw, 100px) 5%", background: hasBg ? (isDark ? "rgba(122,63,209,0.03)" : "rgba(122,63,209,0.02)") : "transparent", borderTop: hasBg ? "1px solid " + border : "none", borderBottom: hasBg ? "1px solid " + border : "none" }}>
      <div className="exhibit-row" style={{ maxWidth: 1300, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "clamp(40px, 6vw, 80px)", alignItems: "center" }}>
        <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ type: "spring", bounce: 0.2, duration: 1.2 }}>
          <BoothGallery images={tier.images} isDark={isDark} border={border} cardBg={cardBg} />
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ type: "spring", bounce: 0.2, duration: 1.2, delay: 0.2 }}
          style={{ display: "flex", flexDirection: "column" }}
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16, marginBottom: 16 }}>
            <h2 style={{ fontFamily: "'Orbitron', sans-serif", fontSize: "clamp(2rem, 4vw, 2.8rem)", fontWeight: 900, color: textMain, lineHeight: 1.1 }}>{tier.title}</h2>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontFamily: "monospace", fontSize: "1.1rem", color: textMuted, marginBottom: 4 }}>{tier.specs}</div>
              <PriceWithAsterisk price={tier.price} color={isDark ? "#f5a623" : "#d98a14"} fontSize="1.4rem" fontWeight={800} />
            </div>
          </div>
          <p style={{ fontSize: "1.2rem", fontWeight: 700, color: textMain, marginBottom: 24, lineHeight: 1.5 }}>{tier.tagline}</p>
          <div style={{ fontSize: "1.05rem", color: textMuted, lineHeight: 1.7, marginBottom: 32, textAlign: "justify", whiteSpace: "pre-line" }}>{tier.description}</div>
          <div style={{ background: cardBg, border: "1px solid " + border, borderRadius: 16, padding: "24px 32px" }}>
          ...
          <motion.button onClick={onOpenModal} className="btn-primary" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
...
function EnquireModal({ booth, onClose, isDark, textMain, border }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok || res.status === 200) { setStatus("Success! We will be in touch shortly."); setTimeout(() => { onClose(); }, 2500); }
      else { setStatus("Error — please try again."); }
    } catch { setStatus("Error — please try again."); }
  };
  const inputStyle = { width: "100%", padding: "12px 16px", borderRadius: "8px", background: isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.03)", border: "1px solid " + border, color: textMain, fontSize: "0.95rem", outline: "none", marginBottom: "16px" };

  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.7)", backdropFilter: "blur(8px)", padding: "24px" }}>
      ...
      >
        <button onClick={onClose} style={{ position: "absolute", top: "20px", right: "24px", background: "transparent", border: "none", color: textMain, fontSize: "1.5rem", cursor: "pointer", opacity: 0.6 }}>✕</button>
        <h3 style={{ fontFamily: "'Orbitron', sans-serif", fontSize: "1.4rem", fontWeight: 800, color: textMain, marginBottom: "8px" }}>
          Enquire about <GradientSpan>{booth.specs}</GradientSpan>
        </h3>
        <p style={{ fontSize: "0.9rem", color: isDark ? "rgba(255,255,255,0.6)" : "rgba(0,0,0,0.6)", marginBottom: "24px" }}>Leave your details below and our partnership team will reach out.</p>
        <form onSubmit={handleSubmit}>
          <div style={{ display: "flex", gap: "16px" }}>
            <input type="text" name="firstName" placeholder="First Name" value={formData.firstName} onChange={handleChange} required style={inputStyle} />
            <input type="text" name="lastName" placeholder="Last Name" value={formData.lastName} onChange={handleChange} required style={inputStyle} />
