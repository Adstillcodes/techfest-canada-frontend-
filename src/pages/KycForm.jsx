...
function FormInput({ num, label, required, dark, error, ...props }) {
  ...
  return (
    <div style={{ marginBottom: "40px", background: dark ? "rgba(255,255,255,0.01)" : "rgba(0,0,0,0.01)", padding: "24px", borderRadius: "16px", border: `1px solid ${dark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.05)"}` }}>
      ...
      <input style={{ width: "100%", background: inputBg, border: `2px solid ${inputBorder}`, color: textMain, padding: "18px 20px", borderRadius: "12px", fontSize: "1.05rem", outline: "none", transition: "all 0.2s", boxShadow: "0 4px 6px rgba(0,0,0,0.02)" }}
...
function CustomDropdown({ num, label, required, name, options, value, onChange, multi = false, dark, error }) {
  ...
  if (multi && value && value.length > 0) displayValue = `${value.length} selected`;
  else if (!multi && value) displayValue = value;
...
function RichTextEditor({ num, label, required, value, onChange, dark, error }) {
  ...
  const exec = (command, val = null) => { document.execCommand(command, false, val); editorRef.current.focus(); handleInput(); };
      ...
      <div style={{ background: inputBg, border: `2px solid ${inputBorder}`, borderRadius: "12px", overflow: "hidden", boxShadow: "0 4px 6px rgba(0,0,0,0.02)" }}>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", padding: "10px", background: bgToolbar, borderBottom: `1px solid ${dark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)"}` }}>
          ...
          <select onChange={(e) => exec("fontName", e.target.value)} style={{ background: "transparent", border: "none", color: textMain, outline: "none", cursor: "pointer" }}>
            <option value="Arial">Sans-Serif</option><option value="Georgia">Serif</option><option value="Courier New">Monospace</option>
          </select>
        ...
        <div ref={editorRef} contentEditable onInput={handleInput} style={{ minHeight: "150px", padding: "20px", color: textMain, outline: "none", fontSize: "1.05rem", lineHeight: "1.6" }} />
      </div>
    </div>
  );
}
...
function ReviewField({ num, label, value, isHtml, dark }) {
  ...
  return (
    <div style={{ marginBottom: "24px" }}>
      <div style={{ fontSize: "0.85rem", textTransform: "uppercase", letterSpacing: "1px", color: "#f5a623", marginBottom: "8px", fontWeight: 700 }}>{num}. {label}</div>
      {isHtml ? (
...
export default function KYCForm() {
  ...
  // Step 0 = Onboarding, Steps 1-5 = Form, Step 6 = Review, Step 7 = Final Email
  const TOTAL_STEPS = 8;
  ...
  useEffect(() => {
    if (typeof window !== "undefined") localStorage.setItem("kycFormDraft", JSON.stringify(formData));
  }, [formData]);
  ...
  const validateStep = () => {
    let required = [];
    if (currentStep === 0) return true; // Onboarding — no validation
    if (currentStep === 1) required = ["primaryFirstName", "primaryLastName", "primaryEmail", "primaryTitle"];
    if (currentStep === 2) required = ["companyName", "countryHQ", "mainOfficeCity", "operationsCanada"];
    if (currentStep === 3) required = ["primaryIndustry", "subIndustry", "productDesc"];
    if (currentStep === 4) required = ["primaryReasons", "successDef", "topOutcomes"];
    if (currentStep === 7) required = ["finalConfirmationEmail"];
  ...
  // Progress starts from step 1 (onboarding doesn't count)
  const progressPercent = currentStep === 0 ? 0 : Math.round(((currentStep - 1) / (TOTAL_STEPS - 2)) * 100);
  ...
  return (
    <div style={{ background: bg, minHeight: "100vh", color: textMain, transition: "background 0.3s ease, color 0.3s ease", position: "relative" }}>
      ...
      {/* STICKY STATUS BAR — hidden on onboarding */}
      {currentStep > 0 && (
      ...
      <div style={{ maxWidth: "1000px", margin: "60px auto 80px", padding: "0 5%" }}>
        ...
        {/* ================= STEP 0: ONBOARDING ================= */}
        {currentStep === 0 && (
          <div style={{ animation: "fadeIn 0.5s ease", textAlign: "center", padding: "clamp(60px,12vw,120px) 0" }}>
            <div style={{ background: "linear-gradient(135deg, rgba(122,63,209,0.12), rgba(245,166,35,0.08))", width: "100px", height: "100px", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 40px", boxShadow: "0 10px 40px rgba(122,63,209,0.15)" }}>
              <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#7a3fd1" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg>
            </div>
            <h1 style={{ fontFamily: "'Orbitron', sans-serif", fontSize: "clamp(2rem,5vw,3.2rem)", fontWeight: 900, marginBottom: "20px", lineHeight: 1.15 }}>
              Welcome to the{" "}
              <span style={{ background: "linear-gradient(135deg, #7a3fd1, #f5a623)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
                TTFC Onboarding Form
              </span>
            </h1>
            <p style={{ color: textMuted, fontSize: "1.15rem", lineHeight: 1.75, maxWidth: 600, margin: "0 auto 20px" }}>
              This form helps us understand your business, objectives, and what you would like to achieve at The Tech Festival Canada 2026.
            </p>
            <p style={{ color: textMuted, fontSize: "0.95rem", lineHeight: 1.7, maxWidth: 520, margin: "0 auto 48px" }}>
              It takes approximately 5–8 minutes to complete. Your progress is saved automatically as you go.
            </p>
            <button onClick={handleNext}
              style={{ background: "linear-gradient(135deg, #7a3fd1, #f5a623)", color: "#fff", padding: "22px 56px", borderRadius: "16px", border: "none", fontFamily: "'Orbitron', sans-serif", fontWeight: 800, fontSize: "1.1rem", letterSpacing: "1.5px", textTransform: "uppercase", cursor: "pointer", boxShadow: "0 10px 40px rgba(122,63,209,0.3)", transition: "transform 0.2s ease" }}
        ...
        {currentStep === 2 && (
          <div style={{ animation: "fadeIn 0.5s ease" }}>
            <div style={{ marginBottom: "50px" }}>
              ...
              <p style={{ color: textMuted, fontSize: "1.1rem" }}>Provide details about your organization.</p>
            </div>
            ...
            <CustomDropdown num="7" label="Country of Headquarters" required name="countryHQ" options={["Canada", "USA", "UK", "Europe", "India", "Middle East", "APAC", "Other"]} value={formData.countryHQ || ""} onChange={handleChange} dark={dark} error={missingFields.includes("countryHQ")} />
            <FormInput num="8" label="City of Main Office" required name="mainOfficeCity" value={formData.mainOfficeCity || ""} onChange={handleChange} dark={dark} error={missingFields.includes("mainOfficeCity")} />
            <FormRadio num="9" label="Do you have operations in Canada?" required name="operationsCanada" options={["Yes", "No", "Planned market entry"]} value={formData.operationsCanada || ""} onChange={handleChange} dark={dark} error={missingFields.includes("operationsCanada")} />
          </div>
        )}
        ...
        {currentStep === 3 && (
          <div style={{ animation: "fadeIn 0.5s ease" }}>
            ...
            <CustomDropdown num="10" label="Primary Industry (Select all that apply)" required name="primaryIndustry" multi options={[
              "Artificial Intelligence", "Quantum Computing", "Robotics", "Cybersecurity and Digital Trust",
              "Climate Technology and Sustainability", "Healthcare", "Financial Services / FinTech",
              "Energy and Utilities", "Manufacturing and Industrial", "Mobility and Transportation",
              "Government and Smart Cities", "Retail and Consumer", "Education",
              "Agriculture and Food", "Telecom", "Defence and Public Safety", "Cross sector / Multi tech"
            ]} value={formData.primaryIndustry} onChange={handleChange} dark={dark} error={missingFields.includes("primaryIndustry")} />
            <CustomDropdown num="11" label="Sub Industry / Category" required name="subIndustry" multi options={[
              "GenAI / LLMs", "AI Infrastructure / Compute", "AI Agents / Automation", "Data / Analytics / MLOps",
              "Quantum Hardware", "Quantum Software / Algorithms", "Robotics Hardware", "Industrial Automation",
              "Humanoid Robotics", "Cybersecurity Software", "Identity / Zero Trust", "Cloud Security",
              "Climate / Clean Energy", "Carbon / ESG / Sustainability Tech", "Smart Infrastructure", "Other"
            ]} value={formData.subIndustry} onChange={handleChange} dark={dark} error={missingFields.includes("subIndustry")} />
            <FormRadio num="12" label="Company Stage (Optional)" name="companyStage" options={["Startup", "Growth Stage", "Scale Up", "Established Enterprise", "Global Corporation"]} value={formData.companyStage || ""} onChange={handleChange} dark={dark} />
            <RichTextEditor num="13" label="Describe your product, solution, or offering" required value={formData.productDesc} onChange={(val) => { setFormData({...formData, productDesc: val}); if(missingFields.includes("productDesc")) setMissingFields(prev => prev.filter(f => f !== "productDesc")); }} dark={dark} error={missingFields.includes("productDesc")} />
          </div>
        )}
        ...
        {currentStep === 4 && (
          <div style={{ animation: "fadeIn 0.5s ease" }}>
            ...
            <CustomDropdown num="14" label="Primary reasons for participating" required name="primaryReasons" multi options={[
              "Brand visibility", "Lead generation", "Sales pipeline creation", "Customer acquisition",
              "Partnership development", "Investor outreach", "Channel / distributor search",
              "Market entry into Canada", "Thought leadership / speaking", "Product launch",
              "PR / media exposure", "Competitive intelligence", "Recruit talent",
              "Meet government / academia / ecosystem leaders", "Existing customer engagement", "Networking", "Other"
            ]} value={formData.primaryReasons} onChange={handleChange} dark={dark} error={missingFields.includes("primaryReasons")} />
            <RichTextEditor num="15" label="What would define success for your participation?" required value={formData.successDef} onChange={(val) => { setFormData({...formData, successDef: val}); if(missingFields.includes("successDef")) setMissingFields(prev => prev.filter(f => f !== "successDef")); }} dark={dark} error={missingFields.includes("successDef")} />
            <CustomDropdown num="16" label="Top success outcomes" required name="topOutcomes" multi options={[
              "Number of qualified meetings", "Number of enterprise buyer meetings",
              "Number of government / public sector meetings", "Number of investor meetings",
              "Number of channel / partner meetings", "Number of booth visits",
              "Number of leads captured", "Number of product demos conducted",
              "Number of speaking attendees reached", "Brand impressions",
              "Press / media exposure", "Social media reach",
              "Strategic partnerships initiated", "Sales opportunities generated",
              "Market insights gathered", "Hiring conversations initiated"
            ]} value={formData.topOutcomes} onChange={handleChange} dark={dark} error={missingFields.includes("topOutcomes")} />
          </div>
        )}
        ...
        {currentStep === 5 && (
          <div style={{ animation: "fadeIn 0.5s ease" }}>
            <div style={{ marginBottom: "50px" }}>
              ...
              <p style={{ color: textMuted, fontSize: "1.1rem" }}>Help us understand your competitive landscape and networking goals.</p>
            </div>
            <RichTextEditor num="17" label="Which competitors or comparable companies do you want to position against or alongside?" value={formData.competitors} onChange={(val) => setFormData({...formData, competitors: val})} dark={dark} />
            <RichTextEditor num="18" label="Who are your top 5 competitors?" value={formData.topFiveCompetitors} onChange={(val) => setFormData({...formData, topFiveCompetitors: val})} dark={dark} />
            <RichTextEditor num="19" label="What are the top 5 Job Titles or Companies you would like to meet at the event?" value={formData.topFiveMeetings} onChange={(val) => setFormData({...formData, topFiveMeetings: val})} dark={dark} />
          </div>
        )}
        ...
        {currentStep === 6 && (
          <div style={{ animation: "fadeIn 0.5s ease" }}>
            ...
            {[
              ...
              { t: "Company Information", s: 2, f: [{ l: "Company", v: formData.companyName }, { l: "HQ", v: formData.countryHQ }, { l: "City", v: formData.mainOfficeCity }, { l: "Canada Ops", v: formData.operationsCanada }] },
              { t: "Business Profile", s: 3, f: [{ l: "Primary Industry", v: formData.primaryIndustry }, { l: "Sub Industry", v: formData.subIndustry }, { l: "Company Stage", v: formData.companyStage }, { l: "Product", v: formData.productDesc, isHtml: true }] },
              ...
              { t: "Market Positioning", s: 5, f: [{ l: "Competitors Positioning", v: formData.competitors, isHtml: true }, { l: "Top 5 Competitors", v: formData.topFiveCompetitors, isHtml: true }, { l: "Top 5 Job Titles / Companies to Meet", v: formData.topFiveMeetings, isHtml: true }] }
            ].map((section, idx) => (
              <div key={idx} style={{ background: dark ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.02)", borderRadius: "20px", padding: "40px", marginBottom: "30px", border: `1px solid ${dark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.05)"}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "30px", borderBottom: `1px solid ${dark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.05)"}`, paddingBottom: "20px" }}>
                  ...
                  <button onClick={() => setCurrentStep(section.s)} style={{ background: dark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.05)", border: "none", color: textMain, fontWeight: 700, padding: "8px 16px", borderRadius: "8px", cursor: "pointer", fontSize: "0.9rem", transition: "background 0.2s" }}
        ...
        {/* NAVIGATION BOTTOM BAR — hidden on onboarding */}
        {currentStep > 0 && (
          <div style={{ display: "flex", gap: "20px", marginTop: "60px", paddingTop: "40px", borderTop: `1px solid ${dark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.05)"}` }}>
            ...
            >← Previous</button>
            <button onClick={handleNext}
              style={{ flex: 2, background: currentStep === 7 ? "linear-gradient(135deg, #7a3fd1, #f5a623)" : btnBg, color: currentStep === 7 ? "#fff" : btnText, padding: "24px", borderRadius: "16px", border: "none", fontWeight: 800, fontSize: "1.1rem", cursor: "pointer", boxShadow: "0 10px 30px rgba(0,0,0,0.15)", transition: "transform 0.2s ease" }}
              onMouseEnter={(e) => e.currentTarget.style.transform = "translateY(-2px)"}
              onMouseLeave={(e) => e.currentTarget.style.transform = "translateY(0)"}
            >
              {currentStep === 6 ? "Review & Confirm" : (currentStep === 7 ? "Submit Application" : "Continue →")}
            </button>
          </div>
        )}
      ...
      {isSubmitted && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", background: dark ? "rgba(6,2,15,0.95)" : "rgba(253,251,250,0.95)", backdropFilter: "blur(20px)", padding: "20px" }}>
          <div style={{ textAlign: "center", animation: "fadeIn 0.5s ease-out", maxWidth: "600px", background: dark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.02)", padding: "60px 40px", borderRadius: "30px", border: `1px solid ${dark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.05)"}`, boxShadow: "0 20px 50px rgba(0,0,0,0.2)" }}>
            ...
            <button onClick={() => { localStorage.removeItem("kycFormDraft"); window.location.href = "/"; }}
