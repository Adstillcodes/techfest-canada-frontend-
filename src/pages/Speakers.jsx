...
var SPEAKERS_QUERY = '*[_type == "speaker"] | order(order asc) { _id, name, title, company, bio, linkedin, image }';

var INDUSTRIES = [
  "Artificial Intelligence & Machine Learning", "Quantum Computing", "Cybersecurity",
  "Cloud Computing & Infrastructure", "Robotics & Automation", "Sustainability & CleanTech",
  "Blockchain & Web3", "Biotechnology & HealthTech", "FinTech & Digital Payments",
  "Space Technology", "EdTech & Future of Work", "Smart Cities & IoT",
  "Defence & Public Safety", "Supply Chain & Logistics Tech", "Enterprise Software & SaaS",
  "Venture Capital & Investment", "Government & Public Policy", "Media & Communications", "Other",
];

var EXPERIENCE_OPTIONS = [
  "First time — never spoken at a conference", "1–5 conferences", "5–10 conferences",
  "10–15 conferences", "15–25 conferences", "25+ conferences",
];
...
function SpeakerApplicationModal(props) {
  ...
  var bg = dark ? "#0d0620" : "#ffffff"; var textMain = dark ? "#ffffff" : "#0d0520";
  ...
  function handleChange(e) { setFormData(function(prev) { return Object.assign({}, prev, { [e.target.name]: e.target.value }); }); setError(""); }

  function handleSubmit(e) {
    e.preventDefault();
    if (!formData.firstName || !formData.lastName || !formData.email || !formData.industry || !formData.jobTitle || !formData.experience) { setError("Please fill in all required fields."); return; }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) { setError("Please enter a valid email address."); return; }
    setLoading(true);
    fetch("https://api.emailjs.com/api/v1.0/email/send", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ service_id: "service_gy3fvru", template_id: "template_ufqzzep", user_id: "gZgYZtLCXPVgUsVj_",
        template_params: { to_email: "baldeep@thetechfestival.com", from_name: formData.firstName + " " + formData.lastName, from_email: formData.email,
          message: "[Speaker Application]\nName: " + formData.firstName + " " + formData.lastName + "\nTitle: " + formData.jobTitle + "\nIndustry: " + formData.industry + "\nExperience: " + formData.experience + "\nLinkedIn: " + (formData.linkedin || "N/A") } }),
    }).then(function(res) { setLoading(false); if (res.ok || res.status === 200) { setSubmitted(true); } else { setError("Something went wrong. Please try again."); }
    }).catch(function() { setLoading(false); setError("Something went wrong. Please try again."); });
  }

  var inputStyle = { width: "100%", padding: "12px 16px", borderRadius: 10, background: inputBg, border: "1px solid " + inputBdr, color: textMain, fontSize: "0.92rem", outline: "none", fontFamily: "inherit", boxSizing: "border-box" };
  var labelStyle = { display: "block", fontSize: "0.72rem", fontFamily: "'Orbitron',sans-serif", fontWeight: 700, letterSpacing: "0.8px", textTransform: "uppercase", color: textMid, marginBottom: 6 };

  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.75)", backdropFilter: "blur(10px)", padding: "20px" }} onClick={function(e) { if (e.target === e.currentTarget) onClose(); }}>
...
function SpeakerCard(props) {
  var speaker = props.speaker; var dark = props.dark;
  ...
  return (
    <Link to={"/speakers/" + speaker._id} className="speaker-card" style={{
      background: cardBg, border: "1px solid " + cardBdr, borderRadius: 20,
      overflow: "hidden", cursor: "pointer", display: "flex", flexDirection: "column",
      textDecoration: "none", color: "inherit",
    }}>
      {/* Photo with hover animation */}
      <div className="speaker-img-wrap" style={{ position: "relative", width: "100%", aspectRatio: "1/1", overflow: "hidden", background: dark ? "#120a22" : "#ede8f7" }}>
        {speaker.image ? (
          <img className="speaker-img" src={urlFor(speaker.image).width(400).height(400).url()} alt={speaker.name}
            style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }}
            onError={function(e) { e.target.style.display = "none"; }} />
        ) : null}
        {/* Purple gradient overlay that appears on hover */}
        <div className="speaker-img-glow" />
        {/* Bottom fade */}
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "40%", background: "linear-gradient(to top, " + (dark ? "rgba(6,2,15,0.85)" : "rgba(255,255,255,0.7)") + ", transparent)", pointerEvents: "none", zIndex: 2 }} />
        {/* View Profile hint */}
        <div className="speaker-view-hint" style={{ position: "absolute", bottom: 14, left: 0, right: 0, textAlign: "center", zIndex: 3 }}>
          <span style={{ fontFamily: "'Orbitron',sans-serif", fontSize: "0.6rem", fontWeight: 800, letterSpacing: "1.5px", textTransform: "uppercase", color: "#ffffff", background: "rgba(122,63,209,0.65)", backdropFilter: "blur(8px)", padding: "5px 14px", borderRadius: 999 }}>View Profile</span>
        </div>
      </div>

      {/* Info */}
      <div className="speaker-info" style={{ padding: "16px 18px 14px", flex: 1, display: "flex", flexDirection: "column" }}>
        <h3 className="speaker-name" style={{ fontFamily: "'Orbitron', sans-serif", fontSize: "0.82rem", fontWeight: 900, color: dark ? "#4ade80" : "#1a9e70", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 3, lineHeight: 1.3 }}>{speaker.name}</h3>
        ...
        {/* LinkedIn */}
        {speaker.linkedin && (
          <span className="speaker-li-btn" onClick={function(e) { e.preventDefault(); e.stopPropagation(); window.open(speaker.linkedin, "_blank"); }} style={{
            display: "flex", alignItems: "center", justifyContent: "center", gap: 7,
            marginTop: 8, padding: "9px 14px", borderRadius: 8,
            background: "#0A66C2", color: "#ffffff", fontSize: "0.72rem", fontWeight: 700, letterSpacing: "0.3px", cursor: "pointer",
          }}>
...
export default function Speakers() {
  ...
  var filtered = useMemo(function() {
    ...
    return speakers.filter(function(s) {
      return (s.name && s.name.toLowerCase().includes(q)) || (s.company && s.company.toLowerCase().includes(q)) || (s.title && s.title.toLowerCase().includes(q)) || (s.bio && s.bio.toLowerCase().includes(q));
    });
  }, [search, speakers]);
  ...
  var purpleRgb = "122, 63, 209"; var orangeRgb = "245, 166, 35";
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        .speakers-page { min-height:100vh; display:flex; flex-direction:column; }
        ...
        .speakers-hero h1 { font-size:clamp(2.4rem,5vw,4rem); font-weight:900; font-family:'Orbitron',sans-serif; line-height:1.05; margin-bottom:1.2rem; }
        .speakers-hero h1 span { color:#f5a623; }
        .spk-sub { font-size:1rem; max-width:520px; margin:0 auto 2.4rem; line-height:1.75; }
        ...
        .stat-cell { display:flex; align-items:center; gap:14px; padding:24px 20px; }
        ...
        .stat-value { font-family:'Orbitron',sans-serif; font-size:1.4rem; font-weight:900; line-height:1; }
        .stat-label { font-size:0.68rem; font-weight:700; text-transform:uppercase; letter-spacing:0.8px; margin-top:4px; line-height:1.3; }
        ...
        .speaker-card:hover {
          ...
          box-shadow: 0 20px 60px rgba(122,63,209,0.25), 0 0 40px rgba(122,63,209,0.08);
        }
        ...
        .speaker-img-glow {
          ...
          background: radial-gradient(ellipse at 50% 60%, rgba(122,63,209,0.45) 0%, rgba(185,158,255,0.15) 40%, transparent 70%);
          opacity: 0;
        ...
        .spk-cta-band { margin:0 5% 5rem; border-radius:24px; padding:3.5rem 4rem; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:2rem; }
        .spk-cta-band h3 { font-size:1.6rem; font-weight:900; font-family:'Orbitron',sans-serif; margin-bottom:0.5rem; }
        .spk-cta-band h3 span { color:#f5a623; }
        .spk-cta-band p { font-size:0.92rem; max-width:420px; line-height:1.65; }
      ...
      <div className="speakers-page">
        ...
        <main style={{ flex: 1 }}>
          <section className="speakers-hero">
            <div className="spk-orb-1" /><div className="spk-orb-2" /><div className="spk-grid-bg" />
          ...
          <div className="speakers-stats">
            {stats.map(function(s) { var Icon = s.icon; return (
              <div className="stat-cell" key={s.label}><div className="stat-icon"><Icon size={18} /></div><div><div className="stat-value">{s.value}</div><div className="stat-label">{s.label}</div></div></div>
            ); })}
          </div>

          {/* Sticky Search */}
          <div style={{ position: "sticky", top: "64px", zIndex: 40, background: dark ? "rgba(6,2,15,0.97)" : "rgba(255,255,255,0.97)", backdropFilter: "blur(18px)", WebkitBackdropFilter: "blur(18px)", borderBottom: "1px solid " + border, padding: "14px 5%" }}>
            <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, padding: "10px 16px", borderRadius: 12, background: dark ? "rgba(255,255,255,0.06)" : "rgba(122,63,209,0.04)", border: "1.5px solid " + (dark ? "rgba(255,255,255,0.10)" : "rgba(122,63,209,0.12)") }}>
                <Search size={16} style={{ opacity: 0.4, flexShrink: 0 }} />
                <input value={search} onChange={function(e) { setSearch(e.target.value); }} placeholder="Search speakers..." style={{ background: "transparent", border: "none", outline: "none", fontSize: "0.88rem", color: textMain, width: "100%", fontFamily: "inherit" }} />
          ...
          <section style={{ padding: "3rem 5%", maxWidth: 1200, margin: "0 auto" }}>
            ...
            {loading ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "4rem 0", gap: 12, opacity: 0.4 }}>
                <div style={{ fontSize: "2rem", animation: "spk-spin 1s linear infinite" }}>⟳</div><p style={{ fontSize: "0.9rem" }}>Loading speakers...</p>
              </div>
            ) : error ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "4rem 0", gap: 12, opacity: 0.4 }}><p style={{ fontSize: "0.9rem", color: "#ff6b6b" }}>{error}</p></div>
            ) : filtered.length === 0 ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "4rem 0", gap: 12, opacity: 0.4 }}>
                <Search size={28} style={{ opacity: 0.3 }} /><p style={{ fontSize: "0.9rem" }}>No speakers match your search.</p>
              ...
              <div className="speakers-grid">{filtered.map(function(speaker) { return <SpeakerCard key={speaker._id} speaker={speaker} dark={dark} />; })}</div>
            )}

            <div style={{ textAlign: "center", padding: "3.5rem 0 1rem" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 12, padding: "16px 36px", borderRadius: 14, background: dark ? "rgba(255,255,255,0.04)" : "rgba(122,63,209,0.04)", border: "1px solid " + cardBdr }}>
          ...
          <section style={{ padding: "5rem 5%", background: dark ? "rgba(122,63,209,0.04)" : "rgba(122,63,209,0.02)", borderTop: "1px solid " + cardBdr, borderBottom: "1px solid " + cardBdr }}>
            <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
              ...
              <p style={{ color: textMid, fontSize: "1rem", lineHeight: 1.8, maxWidth: 520, margin: "0 auto" }}>We are assembling a distinguished advisory council of industry leaders, policymakers, and technology pioneers.</p>
            </div>
          </section>

          <div className="spk-cta-band">
            <div><h3>Want to <span>Speak</span> at TTFC?</h3><p>We're looking for visionary leaders, innovators, and experts to take the stage. Applications for TTFC 2026 are now open.</p></div>
