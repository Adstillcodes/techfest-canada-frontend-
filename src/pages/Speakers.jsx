    ...
    <>
      <style>{`
        ...
        [data-theme="dark"] {
          ...
          --grid-line: rgba(var(--brand-purple-rgb), 0.10);
          --stat-icon-bg: rgba(var(--brand-purple-rgb), 0.15);
          --stat-icon-border: rgba(var(--brand-purple-rgb), 0.25);
          --stat-icon-color: #b99eff;
          --cta-band-glow: rgba(var(--brand-purple-rgb), 0.20);
        }
        ...
        .speakers-hero-orb-1 { position:absolute; pointer-events:none; width:500px; height:500px; border-radius:50%; background:radial-gradient(circle,rgba(var(--brand-purple-rgb),0.25) 0%,transparent 70%); top:-160px; left:-100px; filter:blur(70px); }
        .speakers-hero-orb-2 { position:absolute; pointer-events:none; width:400px; height:400px; border-radius:50%; background:radial-gradient(circle,rgba(var(--brand-orange-rgb),0.12) 0%,transparent 70%); top:-80px; right:-80px; filter:blur(70px); }
        .speakers-hero-grid { position:absolute; inset:0; pointer-events:none; background-image:linear-gradient(var(--grid-line) 1px,transparent 1px),linear-gradient(90deg,var(--grid-line) 1px,transparent 1px); background-size:60px 60px; mask-image:radial-gradient(ellipse 80% 80% at 50% 50%,black 40%,transparent 100%); }
        .speakers-eyebrow { display:inline-flex; align-items:center; gap:8px; background:var(--eyebrow-bg); border:1px solid var(--eyebrow-border); color:var(--brand-purple); padding:5px 16px; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:1.4px; text-transform:uppercase; margin-bottom:1.4rem; }
        .speakers-eyebrow-dot { width:6px; height:6px; border-radius:50%; background:var(--brand-orange); box-shadow:0 0 6px var(--brand-orange); animation:spkPulse 2s ease infinite; }
        ...
        .speakers-hero-sub { font-size:1rem; color:var(--text-muted); max-width:520px; margin:0 auto 2.4rem; line-height:1.75; }
        .speakers-hero-cta { display:inline-flex; align-items:center; gap:8px; background:var(--gradient-brand); color:#fff; border:none; padding:12px 28px; border-radius:999px; font-weight:700; font-size:0.85rem; cursor:pointer; text-decoration:none; transition:opacity 0.2s,transform 0.2s; }
        ...
        .speakers-stats {
          ...
          gap: 1px;
          background: var(--border-main, rgba(122,63,209,0.10));
          border-top: 1px solid var(--border-main, rgba(122,63,209,0.10));
          border-bottom: 1px solid var(--border-main, rgba(122,63,209,0.10));
          margin-bottom: 5rem;
        }
        .stat-cell {
          display: flex; align-items: center; gap: 14px;
          padding: 24px 20px;
          background: var(--bg-card, rgba(122,63,209,0.03));
        }
        .stat-icon { width:44px; height:44px; border-radius:12px; background:var(--stat-icon-bg); border:1px solid var(--stat-icon-border); display:flex; align-items:center; justify-content:center; flex-shrink:0; color:var(--stat-icon-color); }
        .stat-value { font-family:'Orbitron',sans-serif; font-size:1.4rem; font-weight:900; line-height:1; color:var(--text-main); }
        [data-theme="dark"] .stat-value { background:var(--gradient-brand); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; }
        ...
        @media (min-width: 768px) {
          .speakers-stats { grid-template-columns: repeat(4, 1fr); }
        }

        .section-header { text-align:center; margin-bottom:3rem; }
        .section-header h2 { font-size:clamp(1.6rem,3vw,2.4rem); font-weight:900; font-family:'Orbitron',sans-serif; color:var(--text-main); margin-bottom:0.6rem; }
        .section-header h2 span { color:var(--brand-orange); }
        .section-header p { font-size:0.92rem; color:var(--text-muted); max-width:480px; margin:0 auto; line-height:1.7; }
        .section-divider { width:48px; height:3px; background:var(--gradient-brand); border-radius:2px; margin:1rem auto 0; }
        .carousel-section { padding:0 5% 5rem; max-width:1200px; margin:0 auto; }
        .speak-cta-band { margin:5rem 5%; border-radius:24px; padding:3.5rem 4rem; background:var(--bg-card); border:1px solid var(--border-main); display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:2rem; position:relative; overflow:hidden; }
        [data-theme="dark"] .speak-cta-band { background:linear-gradient(135deg,rgba(var(--brand-purple-rgb),0.12) 0%,rgba(var(--brand-orange-rgb),0.06) 100%); }
        .speak-cta-band::before { content:''; position:absolute; inset:0; background:radial-gradient(ellipse 60% 80% at 0% 50%,var(--cta-band-glow),transparent); pointer-events:none; }
        .speak-cta-text h3 { font-size:1.6rem; font-weight:900; font-family:'Orbitron',sans-serif; color:var(--text-main); margin-bottom:0.5rem; }
        .speak-cta-text h3 span { color:var(--brand-orange); }
        .speak-cta-text p { font-size:0.9rem; color:var(--text-muted); max-width:420px; line-height:1.65; }
